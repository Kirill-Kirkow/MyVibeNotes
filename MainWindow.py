...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        uic.loadUi('Qt_widgets/MainWindow.ui', self)  # Импорт ui файла
    ...
    def CreateWindow(self, user_data):  # Метод для отображения окна
        ...
        self.edit_name_page.save_name_button.clicked.connect(lambda: self.UpdateName())  # Назначение действия при нажатии на кнопку сохранения имени
                                                                                         # (кнопка расположена на виджете изменения имени)
    ...
    def CreateNote(self, number, note):  # Метод для отрисовки заметки; кнопок редактирования и удаления заметки и шкалы прогресса на фрейме с заметками
        ...
        if note.type == 'Job_is_done':                           # Создаём
            label.setPixmap(QPixmap('UI/Job_is_done.png'))       # внешний вид
        if note.type == 'Job_in_progress':                       # заметки
            label.setPixmap(QPixmap('UI/Job_in_progress.png'))   # в зависимости от
        if note.type == 'Job_is_cancelled':                      # её
            label.setPixmap(QPixmap('UI/Job_is_cancelled.png'))  # типа
        ...
        edit_note_button.setIcon(QIcon('UI/edit_button.png'))  # Заугржаем иконку кнопки
        ...
        delete_note_button.setIcon(QIcon('UI/delete_button.png'))  # Заугржаем иконку кнопки
    ...
    def GetProgressBarImage(self, type, progress):  # Метод для получения изображения шкалы прогресса в зависимости от типа и прогресса заметки
        ...
        if type == 'Job_is_done':
            progress_bar_pixmap = QPixmap('UI/full_green.png')
        elif progress < 10:
            progress_bar_pixmap = QPixmap('UI/empty.png')
        elif type == 'Job_in_progress':
            if 10 <= progress < 20:
                progress_bar_pixmap = QPixmap('UI/10_yellow.png')
            elif 20 <= progress < 30:
                progress_bar_pixmap = QPixmap('UI/20_yellow.png')
            elif 30 <= progress < 40:
                progress_bar_pixmap = QPixmap('UI/30_yellow.png')
            elif 40 <= progress < 50:
                progress_bar_pixmap = QPixmap('UI/40_yellow.png')
            elif 50 <= progress < 60:
                progress_bar_pixmap = QPixmap('UI/50_yellow.png')
            elif 60 <= progress < 70:
                progress_bar_pixmap = QPixmap('UI/60_yellow.png')
            elif 70 <= progress < 80:
                progress_bar_pixmap = QPixmap('UI/70_yellow.png')
            elif 80 <= progress < 90:
                progress_bar_pixmap = QPixmap('UI/80_yellow.png')
            elif 90 <= progress < 100:
                progress_bar_pixmap = QPixmap('UI/90_yellow.png')
        elif type == 'Job_is_cancelled':
            if 10 <= progress < 20:
                progress_bar_pixmap = QPixmap('UI/10_red.png')
            elif 20 <= progress < 30:
                progress_bar_pixmap = QPixmap('UI/20_red.png')
            elif 30 <= progress < 40:
                progress_bar_pixmap = QPixmap('UI/30_red.png')
            elif 40 <= progress < 50:
                progress_bar_pixmap = QPixmap('UI/40_red.png')
            elif 50 <= progress < 60:
                progress_bar_pixmap = QPixmap('UI/50_red.png')
            elif 60 <= progress < 70:
                progress_bar_pixmap = QPixmap('UI/60_red.png')
            elif 70 <= progress < 80:
                progress_bar_pixmap = QPixmap('UI/70_red.png')
            elif 80 <= progress < 90:
                progress_bar_pixmap = QPixmap('UI/80_red.png')
            elif 90 <= progress < 100:
                progress_bar_pixmap = QPixmap('UI/90_red.png')
